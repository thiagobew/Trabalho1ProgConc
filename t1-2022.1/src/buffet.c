...
void *buffet_run(void *arg) {
    ...
    // Destrói o semáforo de cada comida, cada comida é representada por um semáforo
    // e a quantidade de comida disponível é o valor interno do semáforo, o qual é
    // decrementado por cada aluno que consome o alimento.
    for (int i = 0; i < 5; i++) {
...
void buffet_queue_insert(buffet_t *self, student_t *student) {
    // Verifica para qual lado o estudante irá na fila, essa função é chamada pelo worker_gate após
    // já ter sido garantido que existe lugar vazio nesse buffet.
    if (student->left_or_right == 'L') { /* Se o estudante vai para a fila esquerda */
        // Mutex que bloqueia a primeira posição do array, deixando somente um estudante passar por vez
...
void buffet_next_step(buffet_t *self, student_t *student) {
    ...
    if (student->_buffet_position + 1 < 5) {
        ...
        if (student->left_or_right == 'L') {
            ...
            // Lock no Mutex da próxima posição para esperar o próximo estudante liberá-la
