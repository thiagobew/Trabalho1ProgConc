...
void student_seat(student_t *self, table_t *table) {
  ...
  // Apenas para simulação (tempo para comer)
  msleep(1000);
}
...
void student_serve(student_t *self) {
  ...
  while (self->_buffet_position != -1) {
    if (self->_wishes[self->_buffet_position] == 1) {
      ...
      // Apenas para simulação (tempo para pegar a comida)
      msleep(100);
...
void student_finalize(student_t *self) { free(self); };
...
void *_all_they_come(void *arg) {
  ...
  for (int i = 0; i < number_students; i++) {
    students[i] = student_init(); /* Estudante é iniciado, recebe um ID e
                                     escolhe o que vai comer*/
  }

  for (int i = 0; i < number_students; i++) {
    pthread_create(&students[i]->thread, NULL, student_run,
                   students[i]); /*  Cria as threads  */
  }

  for (int i = 0; i < number_students; i++) {
    pthread_join(students[i]->thread,
                 NULL); /*  Aguarda o término das threads   */
  }
...
/**
 * @brief Função que retorna as escolhas dos alunos, aleatoriamente (50% para
 * cada opção) retornando 1 (escolhido) 0 (não escolhido). É possível que um
 * aluno não goste de nenhuma opção de comida. Nesse caso, considere que ele
 * ainda passa pela fila, como todos aqueles que vão comer.
 * @return int
 */
int _student_choice() {
